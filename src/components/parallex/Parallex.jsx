...
const Parallex = ({ type }) => {
    return (
        <div className="parallex" style={{ background: type === "My skills" ? "linear-gradient(180deg,#0c0c1d,#333357)" : "linear-gradient(180deg,#333357,#0c0c1d)" }}>
            ...
            <div className="mountains" style={{ backgroundImage: "url(./mountains.png)" }}></div>
            <div className="planet" style={{ backgroundImage: type === "My skills" ? "url(./planets.png)" : "url(./sun.png)" }}></div>
            <motion.div className="stars" initial={{ opacity: 0, x: -500 }} whileInView={{ opacity: 1, x: 0, }} transition={{ duration: 1 }} style={{ backgroundImage: "url(./stars.png)" }}></motion.div>
        </div>
    );
}
